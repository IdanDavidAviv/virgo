...
describe('TurnSentinel Sovereignty (TDD: #43)', () => {
    ...
    it('SHOULD reject an injection with a stale turn index (Drift Guard)', async () => {
        ...
        expect(() => TurnManager.updateTurnIndex(sessionPath, { incomingIndex: 5 })).toThrow(/Stale turn index/);
        
        // 3. Accept valid future index
        const index = TurnManager.updateTurnIndex(sessionPath, { incomingIndex: 11 });
