...
describe('TurnSentinel Sovereignty (TDD: #43)', () => {
    ...
    it('SHOULD reject an injection with a stale turn index (Drift Guard)', async () => {
        ...
        fs.writeFileSync(path.join(sessionPath, 'state.json'), JSON.stringify({ current_turn_index: 10 }));
    ...
    it('SHOULD increment turn index automatically if not provided', async () => {
        ...
        // 1. Initial State
        fs.writeFileSync(path.join(sessionPath, 'state.json'), JSON.stringify({ current_turn_index: 5 }));
